...
- 정책을 어느 시점에 검사해야 하는가? 배포 전인가 후인가?
- Rego는 왜 익숙한 언어와 다르게 생겼는가?
- 정책 엔진이 죽으면 클러스터는 어떻게 되는가?
- 이미 돌고 있는 리소스는 어떻게 다루는가?

---

모든 조직에는 정책이 있다. 일부는 거버넌스와 법적 요구사항을 충족하기 위해 필수적이고, 일부는 모범 사례와 조직 관례 준수를 보장하기 위한 것이다.

OPA Gatekeeper는 Kubernetes 클러스터에 정책을 코드로 적용하는 사실상의 표준이다. 이 글은 OPA 공식문서와 Gatekeeper 공식문서를 기반으로 Rego 언어부터 ConstraintTemplate, Mutation, Audit, External Data, Expansion까지 전체 기능을 빠짐없이 다룬다. 각 섹션마다 공식문서에서 확인한 내용을 한국어로 다시 정리했고, 동작 원리 중심 해설과 실무 YAML/Rego 예시를 함께 뒀다.
...
OPA(오-파)는 오픈소스 범용 정책 엔진으로, 스택 전반에 걸쳐 정책 적용을 통합한다.
...
`opa eval`은 Rego 평가를 위한 "만능 도구"이다. `opa run`은 대화형/서버 모드를 제공한다.
...
OPA는 정책 평가를 위해 목적 설계되었으며, 선언형 언어 Rego를 사용한다.

Rego는 "쿼리가 어떻게 실행되어야 하는지"가 아니라 "쿼리가 무엇을 반환해야 하는지"를 명세한다.
...
Rego는 Datalog에서 영감을 받았으며, JSON과 유사한 문서를 처리하도록 확장되었다.

### 2.2 스칼라 값(Scalar Values)

스칼라 값에는 문자열, 숫자, 불리언, null이 포함된다.
...
복합 값에는 배열(순서 있음), 객체(키-값), 집합(순서 없는 고유값)이 포함된다.
...
변수는 입력/출력으로 동시에 작동할 수 있다. 연산자에는 `:=`(로컬 할당), `=`(통합), `==`(비교)가 있다.
...
참조는 점 표기법과, 비표준 키를 위한 괄호 표기법을 지원한다.
...
축약 표현은 컬렉션을 변환/필터링하여 새 컬렉션을 생성하는 문법이다.
...
규칙 유형에는 완전 규칙(단일 값), 부분 규칙(집합/객체), 함수가 있다.
...
증분 정의: 같은 이름의 여러 규칙은 합집합(OR)으로 결합된다.
...
제어 흐름 키워드: if, contains, some, every, not, with, default, else.
...
메타데이터 어노테이션은 주석 내에 YAML 형식으로 작성한다.
...
OPA 사이드카 대비 개선 사항: 확장 가능한 파라미터화 정책 라이브러리, constraint/constraint template/mutation을 위한 네이티브 K8s CRD, Audit, External Data.
...
ConstraintTemplate YAML(k8srequiredlabels) - 이 템플릿은 스키마와 Rego 로직을 정의하며, constraint로 인스턴스화할 수 있는 새 CRD를 생성한다.
...
Constraint YAML(ns-must-have-gk) - 이 constraint는 특정 파라미터와 match 기준으로 템플릿을 인스턴스화한다.
...
match 필드 옵션: kinds, scope(*, Cluster, Namespaced), namespaces(glob), excludedNamespaces(glob), labelSelector, namespaceSelector, name(glob).
...
여러 matcher: AND 로직. 빈 match: 모든 것에 매칭.
...
enforcementAction 옵션: deny(기본값), dryrun, warn.
...
내장 변수: input.review, input.parameters, data.lib, data.inventory.

ConstraintTemplate의 Rego 코드에서 사용할 수 있는 내장 변수는 다음과 같다.

Rego에서 input.review, input.parameters에 접근할 수 있다.
...
v1 ConstraintTemplate은 구조적 스키마를 요구한다(모든 레벨에 type 필드 필요).
...
Rego v1 문법 지원(Gatekeeper 3.19 이상).

레거시 `spec.targets[].rego`가 `code` 배열보다 우선한다.
...
Mutation은 v3.10 이상에서 안정화(Stable) 상태이다.

4가지 CRD: AssignMetadata(레이블/어노테이션 전용, 추가만 가능), Assign(metadata 외부), ModifySet(리스트 집합 연산), AssignImage(이미지 도메인/경로/태그).
...
AssignMetadata는 레이블/어노테이션 전용이며, 기존 값을 수정할 수 없고 추가만 가능하다.
...
Assign은 metadata 외부의 필드를 변경한다. AssignMetadata를 제외한 모든 mutation CRD에는 applyTo가 필수이다.
...
location 경로 파싱: `spec.containers[name:foo].imagePullPolicy`. 와일드카드: `spec.containers[name:*].imagePullPolicy`.
...
pathTests: MustExist(존재해야 함), MustNotExist(존재하지 않아야 함) 조건.
...
fromMetadata: namespace 또는 name 필드를 참조할 수 있다.
...
ModifySet은 리스트 집합 연산을 수행한다. 연산: merge(기본값, 없으면 삽입), prune(제거).
...
AssignImage는 이미지의 도메인/경로/태그를 개별적으로 변경한다.
...
match 기준: scope, kinds, labelSelector, namespaces, namespaceSelector, excludedNamespaces, name.
...
기존 리소스를 constraint에 대해 주기적으로 평가한다.
...
--constraint-violations-limit(기본값: 20). --audit-interval(기본값: 60초, 0=비활성화). --audit-from-cache=true(informer 캐시 사용). --audit-match-kind-only=true.
...
Prometheus 메트릭: gatekeeper_audit_last_run_time, gatekeeper_audit_last_run_end_time, gatekeeper_violations.
...
UserInfo 제약: audit 중에는 userInfo가 비어있다.
...
기본값: `failurePolicy: Ignore`(webhook 에러 시 constraint가 적용되지 않는다).
...
Kubernetes가 일반적으로 제공하는 자가 복구를 방해할 수 있는 순환 종속성이 발생한다.
...
Admission Deadlock 시나리오: 모든 노드 삭제 -> Gatekeeper 종료 -> 새 노드 생성 불가 -> 복구 불가.
...
복구: ValidatingWebhookConfiguration을 삭제한다.
...
운영에서는 대부분의 경우 `failurePolicy: Ignore`(fail-open)를 유지하면서 audit으로 위반을 탐지하는 전략이 안전하다. 금융/의료 등 규정 준수가 필수인 환경에서만 fail-closed를 고려하되, 반드시 데드락 완화 전략을 함께 구성해야 한다.
...
개선 사항에는 확장 가능한 파라미터화 정책 라이브러리가 포함된다.
...
모든 REST 서버는 SLO를 충족하기에 충분한 고가용성 인프라를 필요로 한다.
...
--constraint-violations-limit(기본값: 20).
...
gator는 Gatekeeper ConstraintTemplate과 Constraint를 로컬 환경에서 평가하는 도구이다.
...
확장 가능한 파라미터화 정책 라이브러리, constraint/constraint template/mutation을 위한 네이티브 K8s CRD.
...
**언제 검사해야 하는가.** 배포 전이 원칙이다. Admission 단계에서 막으면 잘못된 리소스가 애초에 클러스터에 들어오지 않는다. 다만 그것만으로는 부족하다. **정책을 나중에 추가하면 이미 들어와 있는 리소스는 그 검사를 안 거쳤다.** 그래서 주기적으로 전체를 다시 검사하는 Audit이 함께 필요하다.

**Rego가 왜 낯선가.** 명령형 언어가 아니라 선언형 질의 언어이기 때문이다. "이렇게 하라"가 아니라 "이런 것이 존재하는가"를 쓴다. 규칙 안의 표현식은 전부 참이어야 하고(AND), 같은 이름의 규칙이 여러 개면 하나만 참이어도 된다(OR). **반복문 대신 변수를 쓰면 컴파일러가 알아서 모든 경우를 훑는다.** 이 모델을 이해하기 전까지는 문법이 계속 어색했다.

**정책 엔진이 죽으면.** `failurePolicy`가 정한다. `Fail`이면 웹훅에 못 닿을 때 요청 자체가 거부되고, `Ignore`면 통과된다. **`Fail`이 안전해 보이지만 위험하다.** 정책 엔진 파드를 띄우는 것조차 웹훅을 거쳐야 하면 스스로를 못 살리는 상태가 된다. 대부분의 환경에서는 `Ignore`로 두고 Audit으로 잡는 편이 낫고, 반드시 막아야 하는 환경이라면 네임스페이스 예외를 함께 설계해야 한다.
