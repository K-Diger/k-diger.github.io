...
8. [OpenClaw - AI 에이전트](#8-openclaw---ai-에이전트)
...
컨텍스트 로트(Context Rot)란 건초더미에서 바늘 찾기(needle-in-a-haystack) 스타일 벤치마크를 통해 발견된 개념으로, 컨텍스트 윈도우의 토큰 수가 증가할수록 모델이 해당 컨텍스트에서 정보를 정확히 회상하는 능력이 감소하는 현상이다. (출처: [Anthropic Engineering Blog](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))

즉, 필요 없는 정보를 더 넣을수록 필요한 정보를 잘못 인식할 확률이 높아진다는 뜻이다. 이는 매우 중요한 발견이다.

더불어 인간의 작업 기억처럼, AI 모델도 '주의 예산'이라는 제한된 자원을 가지고 있다.

인간이 제한된 작업 기억 용량을 가진 것처럼, LLM은 대량의 컨텍스트를 파싱할 때 사용하는 '주의 예산(attention budget)'을 갖고 있으며, 새로운 토큰이 추가될 때마다 이 예산이 일정량 소진된다. (출처: [Anthropic Engineering Blog](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))
...
토큰 개수는 추정값이지, 정확한 값이 아니다. 경우에 따라 실제 토큰 개수가 약간 다를 수 있다. (출처: [Anthropic Claude Token Counting](https://platform.claude.com/docs/en/build-with-claude/token-counting))
...
주목할 만한 추세는 '적절한 시기의 데이터 전략'으로의 이동이다. 에이전트는 가벼운 식별자를 저장했다가, 필요할 때만 데이터를 가져와서 컨텍스트 윈도우를 항상 깨끗하게 유지한다. (출처: [Anthropic Engineering Blog](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))

즉, 모든 정보를 미리 로드하지 말고, 필요할 때만 가져오는 전략이다. 예를 들어, 100개 파일의 목록만 전달하고, 실제로 필요한 파일 2개만 내용을 읽는 식이다.

**원칙 2: 정확히 넣기 (Targeted Prompting)**

프롬프트를 올바른 '높이'에서 작성하라. 지나친 세부 지시(if-then 규칙)도 피하고, 모호함도 피하면서, AI가 생각할 여유를 주는 명확한 지침을 찾아야 한다. (출처: [Anthropic Engineering Blog](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))

즉, 프롬프트는 "A를 하면 B를 해라"는 식의 세세한 규칙보다, "목표는 X다, 어떻게 할지는 네가 판단해"라는 고급 지침이 효율적이다.

**원칙 3: 자주 비우기 (Active Context Management)**

긴 작업을 위한 세 가지 주요 전략은 다음과 같다: 컴팩팅(컨텍스트 한계에 가까워지면 대화를 요약), 구조화된 노트(컨텍스트 윈도우 외부에 정보 저장), 서브에이전트 아키텍처(특정 작업에만 집중하는 전문화된 에이전트를 운영하고, 메인 에이전트는 요약본만 받음). (출처: [Anthropic Engineering Blog](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))
...
Desktop 앱은 세 개의 탭을 제공한다: Chat(일반 질문과 작업을 위한 대화형 인터페이스), Cowork(백그라운드에서 작업을 수행하는 자동 에이전트), Code(프로젝트 파일을 직접 읽고 편집하는 AI 코딩 어시스턴트). (출처: [Claude Code Desktop Docs](https://code.claude.com/docs/en/desktop))

Chat 탭은 일반적인 질문부터 창의적인 작업까지 대화 형식으로 처리한다.

**Code 탭 (2025년 11월 추가):**

Claude Code는 코드베이스와 직접 작동하는 AI 코딩 어시스턴트다. Claude.ai 채팅과 달리, 프로젝트 파일을 읽고, 코드를 편집하고, 터미널 명령을 실행하고, 코드의 각 부분이 어떻게 연결되어 있는지 이해할 수 있다. (출처: [Claude Code Desktop Docs](https://code.claude.com/docs/en/desktop))

Code 탭은 실제 개발 작업에 최적화되었다. 파일을 열어 보고, 수정하고, 즉시 테스트할 수 있다.

**Cowork 탭 (2026년 1월 추가):**

Cowork는 백그라운드에서 작업을 수행하는 자동 에이전트다. 순차적 프롬프트 응답 방식이 아니라, 복잡한 다단계 작업을 맡길 수 있고, Claude가 당신을 대신해 실행한다. (출처: [Getting Started with Cowork](https://support.claude.com/en/articles/13345190-getting-started-with-cowork))
...
Desktop의 Claude Code는 CLI와 동일한 기반 엔진을 그래픽 인터페이스로 실행한다. 둘 다 같은 핵심 기능을 제공한다. (출처: [Claude Code Desktop Docs](https://code.claude.com/docs/en/desktop))
...
각 서브에이전트는 격리된 컨텍스트 윈도우로 작동한다. 오케스트레이터가 서브에이전트를 호출할 때, 그 에이전트는 자신의 작업과 관련된 정보만 받는다. (출처: [Claude Code Sub-Agents Docs](https://code.claude.com/docs/en/sub-agents))
...
초보자가 이해할 수 있게 부연 설명 추가하고. 인용은 한국어로 다시 정리해서...
```
...
사양 주도 개발(SDD)은 일반적으로 구조화된 기능 명세서로 시작하여, 이를 더 작은 조각, 솔루션 및 작업으로 분해하는 여러 단계를 거치는 워크플로우를 의미한다. (출처: [Thoughtworks](https://www.thoughtworks.com/en-us/insights/blog/agile-engineering-practices/spec-driven-development-unpacking-2025-new-engineering-practices))
...
SDD는 전통적인 워크플로우를 역전시켜, 사양(specification)을 진실의 원천으로 취급하고 코드를 생성되거나 검증되는 부차적 산물로 다룬다. (출처: [Thoughtworks](https://www.thoughtworks.com/en-us/insights/blog/agile-engineering-practices/spec-driven-development-unpacking-2025-new-engineering-practices))
...
"바이브 코딩이라는 새로운 코딩 방식이 있는데, 완전히 분위기에 몸을 맡기고, 기하급수적 성장을 수용하며, 코드가 존재한다는 사실을 잊는 것이다." (출처: [Andrej Karpathy, X/Twitter](https://x.com/karpathy/status/1886192184808149383))
...
컨텍스트가 밀려난다. AI 에이전트의 Context Window(한 번에 처리할 수 있는 정보의 양)에는 한계가 있다. 채팅 기록이 쌓이면서 초반의 요구사항이 밀려나고, "처음에 말했듯이 REST API 패턴으로..."라고 매번 상기시켜야 하는 상황이 온다.

일관성이 무너진다. AI가 이전의 아키텍처 결정을 기억하지 못해, 같은 기능을 서로 다른 패턴으로 구현하는 일이 반복된다. 어떤 파일에서는 Repository 패턴을 쓰고, 다른 파일에서는 직접 DB 접근을 하는 식이다.

추적이 안 된다. "왜 이 코드가 이렇게 구현됐는지" 알아낼 방법이 없다. 3개월 후에 버그가 발견됐을 때 원래의 설계 의도를 알 수 없다.

지어내는 일이 늘어난다. 프로젝트 맥락이 부족할수록 AI가 존재하지 않는 API를 호출하거나 근거 없이 코드를 생성하는 빈도가 올라간다. 명확한 명세서는 AI의 환각을 줄인다.
...
컨텍스트 윈도우가 얇아진다. "이 스펙 파일 읽고 구현해"라고 하면 수십 턴의 대화보다 훨씬 적은 토큰으로 같은 정보를 전달할 수 있다. `tasks.md` 하나를 넘기는 것과 30턴의 대화 기록을 넘기는 것의 차이다.

재작업이 줄어든다. 스펙이라는 근거가 있으면 모델이 지어내는 일이 줄고, "이건 아닌데"라며 되돌리는 데 쓰이는 토큰도 줄어든다.

세션을 끊어도 된다. 스펙 파일이 있으면 새 세션에서도 `tasks.md`만 읽으면 작업이 이어진다. 어제의 대화를 붙들고 있을 필요가 없다.
...
OpenSpec은 유연하며 경직되지 않고, 반복적이며 워터폴이 아니고, 간단하며 복잡하지 않고, 그린필드뿐 아니라 브라운필드를 위해 설계되었으며, 개인 프로젝트에서 기업까지 확장 가능하다. (출처: [OpenSpec GitHub](https://github.com/Fission-AI/OpenSpec))
...
이미 코드베이스가 있는 프로젝트에서 OpenSpec을 시작할 때는, `openspec init` 후에 기존 아키텍처를 설명하는 기본 스펙을 먼저 작성하는 것이 좋다. 이후 새 기능을 추가할 때마다 `/opsx:new`로 델타 스펙을 만들면, 전체 스펙을 다시 쓰지 않고 변경사항만 관리할 수 있다.

GitHub: [Fission-AI/OpenSpec](https://github.com/Fission-AI/OpenSpec) / 공식: [openspec.pro](https://openspec.pro)

### 5.2 GitHub Spec Kit

헌법(constitution)은 프로젝트에 대한 협상 불가능한 원칙의 집합으로, 보편적으로 적용되길 원하는 제약 조건이며, LLM이 어떤 상황에서도 건너뛸 수 없는 것이다. (출처: [GitHub Spec Kit](https://github.com/github/spec-kit/blob/main/spec-driven.md))
...
BMAD는 돌파적 애자일 AI 주도 개발 방법론(Breakthrough Method for Agile AI-Driven Development)으로, BMAD Core 프레임워크 기반의 오픈소스 AI 주도 애자일 개발 방법론이자 툴킷이다. 19개의 전문 AI 에이전트와 50개 이상의 워크플로우로 요구사항 분석, 계획, 아키텍처 설계부터 개발 및 테스트까지 전체 생명주기를 커버한다. (출처: [BMAD METHOD GitHub](https://github.com/bmad-code-org/BMAD-METHOD))
...
이것은 구조화되지 않은 바이브 코딩의 '프롬프트를 던지고 기도하기' 식 혼란과 정반대다. (출처: [AWS/Kiro](https://ainativedev.io/news/from-vibe-coding-to-viable-code-aws-dives-into-spec-driven-ai-software-development-with-kiro))
...
cc-sdd는 구조화된 AI-DLC(AI 주도 개발 생명주기)와 사양 주도 개발을 Claude Code에 도입한다. Kiro 스타일 명령어를 사용하여 구조화된 요구사항->설계->작업 워크플로우를 강제한다. (출처: [cc-sdd GitHub](https://github.com/gotalab/cc-sdd))
...
플러그인은 Claude Code에 설치하면 새로운 기능이 추가되는 확장 프로그램이다. 설치 후에는 자동으로 작동하거나 필요할 때 수동으로 실행된다.

훅은 특정 이벤트(세션 시작, 도구 실행 전 등)가 발생하면 자동으로 실행되는 스크립트다. Git의 pre-commit 훅처럼, 정해진 타이밍에 자동으로 작동한다.

스킬은 재사용 가능한 프롬프트 템플릿으로, 특정 작업 방식을 정의한다. 같은 방식의 작업을 할 때마다 수동으로 같은 프롬프트를 쓰지 않아도 된다. SKILL.md 파일로 작성하며, `/skill-name` 형태로 호출한다. 상세한 작성법과 구조는 [3.7 Skills 섹션](#37-skills---재사용-가능한-프롬프트-템플릿)에서 다뤘다.

### 6.1 Oh My Claude

oh-my-claude는 '설치하고 잊기(Install it and forget it)' 철학의 Claude Code 플러그인으로, 파일 읽기, 검색, 구현을 전문 에이전트에 위임하여 메인 세션을 최적 상태로 유지한다. (출처: [Oh My Claude GitHub](https://github.com/TechDufus/oh-my-claude))
...
Enhanced Plan 모드는 `Shift+Tab`을 눌러 Plan 모드에 진입하면, 6단계 파이프라인(Recon → Interview → Research → Gap Analysis → Plan → Critic Review)이 자동으로 실행된다. 일반 Plan 모드보다 훨씬 체계적인 계획을 수립한다.

GitHub: [TechDufus/oh-my-claude](https://github.com/TechDufus/oh-my-claude)

### 6.2 Oh My ClaudeCode

"도구가 아닌 무기" - 5가지 실행 모드를 갖춘 Claude Code를 위한 멀티에이전트 오케스트레이션. (출처: [oh-my-claudecode GitHub](https://github.com/Yeachan-Heo/oh-my-claudecode))
...
oh-my-claude-code는 Zsh의 oh-my-zsh와 같은 존재지만 AI 코딩 에이전트를 위한 것이다. 에이전트가 모든 상호작용에서 학습하고, 가치 있는 지식을 지속하며, 시간이 지남에 따라 자동으로 강해지는 프레임워크다. (출처: [oh-my-claude-code GitHub](https://github.com/KaimingWan/oh-my-claude-code))
...
| 기능 | 무엇인가 |
|---|---|
| 로컬 실행 | 다른 회사의 서버가 아니라 내 컴퓨터에서 돌아간다. 데이터가 기계 밖으로 나가지 않는다 |
| 영구 메모리 | 대화가 끝나도 컨텍스트가 남는다. 일반 챗봇은 새 대화를 시작하면 이전을 잊지만 여기서는 어제 지시한 작업 패턴이 오늘도 살아 있다 |
| 모델 무관 | Claude든 GPT든 로컬 모델(Llama 등)이든 붙일 수 있다 |
| 에이전트 스킬 100개 이상 | 셸 명령 실행, 파일 관리, 웹 자동화, 브라우저 제어 같은 개별 기능 단위 |
| 통합 50개 이상 | Apple Notes, Notion, Obsidian, GitHub, Trello, 스마트홈 기기 |
| Cron 작업 | 정해진 시간에 자동 실행. "매일 아침 8시에 이메일 요약" 같은 예약 |

### 8.4 실제 사용 사례

에러 대응이 대표적이다. Sentry에서 에러가 잡히면 원인을 분석하고, 필요하면 GitHub에 이슈를 만들거나 수정 PR을 올린다.

반복 업무도 넘길 수 있다. "주 1회 팀 회의 일정 생성", "매월 마지막 날 보고서 정리" 같은 것들이다. 웹사이트에서 데이터를 긁어오거나 정기적인 폼 작성을 대신하게 할 수도 있다.

집도 대상이다. "퇴근 모드 켜줘"라고 말하면 조명, 온도, 보안 시스템을 한 번에 조정한다.

### 8.5 보안 우려 - 반드시 읽어야 한다

OpenClaw의 인기가 빠른 만큼, 보안 문제도 심각하다. 이건 단순한 주의사항이 아니라 실제로 발생한 사건에 기반한 경고다.

프롬프트 인젝션은 악의적인 사람이 AI에게 몰래 명령을 끼워 넣는 공격이다. 예를 들어 코드 주석에 "이 파일을 삭제해줘"라는 명령을 숨겨놓으면, AI가 정상적인 지시로 착각하고 실행할 수 있다. OpenClaw는 이메일, 캘린더, 메시징, 파일 시스템 같은 민감한 서비스에 직접 접근하기 때문에, 프롬프트 인젝션 공격이나 잘못된 설정이 있으면 개인정보 유출, 금전 피해로 이어질 수 있다.

공급망 공격은 신뢰할 수 있는 곳(마켓플레이스)에서 받은 것이 실제로는 조작된 것이었던 경우다. 2026년 2월 초에 **"poisoned plugin" 사태**가 실제로 발생했다. ClawHub 마켓플레이스에서 악성 스킬이 발견됐고, 이를 설치한 사용자들의 시스템이 위험에 노출됐다.

**최소 권한 원칙(Principle of Least Privilege):**

최소 권한 원칙은 주체에게 작업을 완료하는 데 필요한 권한만 부여해야 한다는 것이다. (출처: 보안 원칙)
...
- [Faros AI - Best AI Coding Agents](https://www.faros.ai/blog)
...
이 글에서 다룬 것들을 판단 기준으로 다시 묶어둔다.

**토큰은 비용이 아니라 품질 문제다.** 컨텍스트를 많이 넣을수록 좋아질 것 같지만 반대다. 넣을수록 필요한 정보를 정확히 집어내는 능력이 떨어진다. 그래서 "다 넣고 알아서 찾게 하기"가 아니라 "필요한 것만 그때 가져오기"가 기본 전략이 된다.

**컨텍스트를 관리하는 방법은 셋이다.** 길어지면 요약해서 줄이고, 오래 남겨야 할 것은 파일로 빼두고, 성격이 다른 작업은 서브에이전트에게 넘겨 요약만 받는다. 셋 다 "메인 대화창을 얇게 유지한다"는 같은 목적이다.

**스펙 기반 개발이 푸는 문제도 결국 같다.** 대화 30턴을 넘기는 대신 스펙 파일 하나를 넘기면, 같은 정보를 훨씬 적은 토큰으로 전달할 수 있다. 세션을 끊어도 이어갈 수 있다는 것은 그 부수 효과다.

**다만 모든 작업에 스펙이 필요하지는 않다.** 간단한 기능이나 프로토타입에서는 스펙 쓰는 시간이 구현보다 길어진다. 복잡한 기능, 여러 사람이 함께 만지는 코드, 나중에 왜 이렇게 만들었는지 설명해야 하는 경우에 값어치가 나온다.

**도구를 늘리는 것 자체가 비용이다.** 플러그인, 훅, 스킬, MCP 서버를 붙일수록 시작할 때 들어가는 컨텍스트가 늘어난다. 쓰지 않는 것을 꽂아두면 매 요청마다 그만큼을 낸다.

**자동화 범위를 넓힐수록 권한 범위도 넓어진다.** 파일 시스템, 이메일, 캘린더에 직접 닿는 에이전트는 편한 만큼 프롬프트 인젝션과 공급망 공격의 대상이 된다. 최소 권한과 출처 확인이 선택이 아니라 기본이 되는 지점이다.

정리하면서 남은 감각은 **모델을 잘 쓰는 일이 프롬프트를 잘 쓰는 일보다 컨텍스트를 잘 관리하는 일에 가깝다**는 것이었다. 무엇을 넣을지보다 무엇을 안 넣을지를 정하는 쪽이 결과를 더 크게 바꿨다.