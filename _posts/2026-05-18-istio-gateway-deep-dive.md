...
- [Kubernetes Gateway API - HTTPRoute](https://gateway-api.sigs.k8s.io/guides/http-routing/)
...
- Ingress로 부족해서 Gateway가 나온 것인가?
- Gateway API는 또 무엇이고 기존 것들을 대체하는가?
- 나가는 트래픽까지 게이트웨이로 모으는 이유는 무엇인가?
- TLS 종료를 어디서 할지는 무엇을 보고 정하는가?
...
Gateway는 메시의 경계에서 동작하며 수신되는 HTTP/TCP 연결을 받는 로드밸런서를 기술한다.

노출할 포트와 프로토콜을 구성하지만, 트래픽 라우팅 구성은 포함하지 않는다.
...
Istio는 자체 트래픽 관리 API와 함께 Kubernetes Gateway API를 지원하며, 향후 이를 기본 트래픽 관리 API로 만들 계획이다.
...
Gateway API에서는 각 프로토콜 유형이 HTTPRoute, TCPRoute 등 자체 리소스를 가진다.
...
Istio API에서 Gateway는 이미 배포된 기존 gateway Deployment/Service를 구성한다.

Kubernetes Gateway API에서는 Gateway 리소스가 gateway를 구성하고 배포까지 모두 수행한다.
...
Gateway는 메시의 경계에서 동작하며 수신되는 HTTP/TCP 연결을 받는 로드밸런서를 기술한다. 노출할 포트와 프로토콜을 구성하지만, 트래픽 라우팅 구성은 포함하지 않는다.
...
인그레스 트래픽의 라우팅은 내부 서비스 요청과 정확히 동일한 방식으로 라우팅 규칙을 사용하여 구성된다.

이것이 Istio의 핵심 설계 철학이다. 외부에서 들어오는 트래픽이든 내부 서비스 간 트래픽이든, 동일한 VirtualService 라우팅 규칙 체계를 사용한다.

VirtualService는 Istio 서비스 메시 내에서 요청이 서비스로 라우팅되는 방법을 구성할 수 있게 한다.
...
VirtualService에는 어떤 Gateway 리소스가 트래픽을 라우팅할 수 있는지 지정하는 `gateways` 목록이 포함된다.
...
TLS Secret에는 반드시 `tls.key`와 `tls.crt`라는 키가 포함되어야 한다. Mutual TLS의 경우 `ca.crt` 키도 필요하다.
...
여기서 걸리기 쉬운 것이 있다. `credentialName`으로 참조하는 Secret은 Gateway Pod가 위치한 네임스페이스(일반적으로 `istio-system`)에 존재해야 한다. SDS(Secret Discovery Service)를 통해 Envoy에 동적으로 로드되므로, Secret 변경 시 Gateway Pod를 재시작할 필요가 없다.
...
Gateway API는 풍부한 라우팅 기능을 제공하지만, 아직 Istio의 기능 세트를 100% 커버하지는 못한다.
...
Gateway API는 parentRef를 클러스터 내부 서비스로 참조하도록 구성하여 메시 트래픽을 구성하는 데에도 사용할 수 있다.
...
ServiceEntry를 통해 외부 서비스 트래픽 관리, 외부 대상에 대한 재시도/타임아웃/장애 주입, VM 통합이 가능해진다.
...
DestinationRule은 VirtualService 라우팅 이후에 적용된다.
...
**Ingress로 부족한가.** 부족하다. Ingress는 HTTP 경로와 호스트로 라우팅하는 것까지만 표준으로 정해져 있다. 가중치 분배, 헤더 기반 라우팅, 재시도, 타임아웃 같은 것은 **구현체마다 다른 애노테이션으로 표현**해야 한다. 그래서 Ingress를 쓰면 컨트롤러를 바꿀 때 애노테이션을 전부 다시 써야 한다.

**Gateway API는 무엇인가.** Ingress의 그 한계를 표준 안으로 끌어들인 것이다. 애노테이션으로 밀어 넣던 것들이 정식 필드가 됐고, **역할이 분리된 것이 큰 차이다.** 클러스터 관리자가 `GatewayClass`와 `Gateway`로 입구를 정의하고, 애플리케이션 팀이 `HTTPRoute`로 자기 경로만 붙인다. 하나의 큰 Ingress 리소스를 여러 팀이 함께 고치던 문제가 없어진다.

Istio의 Gateway는 Gateway API가 나오기 전에 같은 문제를 Istio 방식으로 푼 것이다. **지금 새로 만든다면 Gateway API 쪽이 방향에 맞고**, 기존 것은 유지하면서 점진적으로 옮기는 것이 현실적이다.

**나가는 트래픽을 왜 모으는가.** 이유가 셋이다. 외부로 나가는 통신을 한 곳에서 통제할 수 있고, 그 지점에서 로그를 남길 수 있고, **방화벽 정책을 그 노드의 IP 하나로 줄일 수 있다.** 폐쇄망처럼 밖으로 나가는 경로가 통제 대상인 환경에서 마지막 항목이 특히 크다.
