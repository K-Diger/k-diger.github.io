...
etcd와 함께 제공되는 기본 클라이언트는 `etcdctl`입니다. 이는 etcd의 명령줄 클라이언트로, 키-값 쌍을 저장하고 검색하는 데 사용할 수 있습니다.
...
- etcd는 분산형, 신뢰성 있는 키-값 저장소입니다
- 전통적인 관계형 데이터베이스와 달리 유연한 구조를 제공합니다
- etcdctl은 etcd와 상호 작용하기 위한 명령줄 도구입니다
- API 버전(v2 또는 v3)에 따라 명령어 구문이 달라집니다
- 쿠버네티스에서 etcd는 클러스터의 상태 정보를 저장하는 중요한 역할을 합니다

다음 글에서는 쿠버네티스에서 etcd가 어떻게 사용되는지, 그리고 고가용성(HA) 환경에서 etcd를 설정하는 방법에 대해 알아보겠습니다.

## 참고 자료

- [etcd 공식 문서](https://etcd.io/docs/)
- [쿠버네티스 공식 문서 - etcd](https://kubernetes.io/ko/docs/tasks/administer-cluster/configure-upgrade-etcd/)
- [CNCF 프로젝트 - etcd](https://www.cncf.io/projects/etcd/)
