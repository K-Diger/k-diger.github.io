...
- 한쪽(Higress)은 helm 설치 한 번으로 AI 기능이 완결되고, AI 전용 플러그인 폭(17종 이상)과 지원 LLM 프로바이더 수가 가장 넓다. 시맨틱 캐시, RAG, 검색 주입까지 게이트웨이 안에서 끝난다. 대신 플러그인을 원격 OCI 레지스트리에서 받아오는 구조라, **레지스트리 접근이 막히면 에러 없이 플러그인만 빠진 채 트래픽이 그대로 통과했다(failopen).** 사내 TLS 인터셉션 환경에서 실제로 이 상황을 겪었다 — 플러그인이 죽은 걸 알아채려면 프록시 설정 덤프를 직접 열어봐야 했다.
