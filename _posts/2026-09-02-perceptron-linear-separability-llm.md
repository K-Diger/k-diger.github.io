...
```mermaid
flowchart TB
    subgraph S0["1~2절: 신경망의 최소 단위"]
        N["뉴런<br/>가중합으로 켜짐/꺼짐을 정한다"]
        NN["신경망<br/>뉴런을 층으로 쌓는다"]
        N --> NN
    end

    subgraph S1["6절: 토크나이저, 사전 찾아보기라 신경망 아님"]
        TXT["텍스트"]
        TID["토큰 ID"]
        TXT -- "encode()" --> TID
    end

    subgraph S2["7절: 임베딩, 표 조회"]
        VEC["임베딩 벡터<br/>실제 의미를 담은 좌표"]
    end

    subgraph S3["8절: Transformer, 어텐션으로 계산하는 진짜 신경망"]
        ENC["인코더<br/>전체를 한 번에 봄"]
        DEC["디코더<br/>한 단어씩 순서대로"]
    end

    subgraph S4["9절: 어느 부품만 쓰느냐로 모델이 갈림"]
        BERT["BERT<br/>빈칸 채우기"]
        GPT["GPT<br/>다음 단어 예측"]
    end

    subgraph S5["10절: 다시 텍스트로"]
        NEXT["다음 토큰 ID"]
        OUT["생성된 텍스트"]
        NEXT -- "decode()" --> OUT
    end
