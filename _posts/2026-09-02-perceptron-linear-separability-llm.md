...
- 프롬프트 엔지니어링, 컨텍스트, RAG, WebFetch, WebSearch 같은 도구가 맞물려서 자율적으로 사용자에게 응답하는 에이전트는 어떻게 동작하는가
...
실제 뉴런에는 활성화 함수가 하나 더 붙는다. 가중합 이후에 씌우는 함수다. 원래 퍼셉트론이 쓰는 계단 함수(step)를 왜 그대로 안 쓰는가. 계단 함수는 0 아니면 1만 나오고 기울기가 없어서, 오차를 거꾸로 흘려보내 가중치를 조금씩 고치는 경사하강법을 적용할 수 없다. 계단 함수 위에서는 경계 지점 딱 하나만 빼면 가중치를 아무리 살짝 옮겨도 출력이 그대로라서, 어느 방향으로 옮겨야 나아지는지 전혀 느낄 수 없다. 그래서 기울기가 있는 함수로 바꾼다. 시그모이드(sigmoid)는 입력이 아무리 작거나 커도 출력을 0과 1 사이로 완만한 S자 곡선을 그리며 눌러 담는 함수고, ReLU는 입력이 0보다 작으면 0을, 0보다 크면 입력을 그대로 내보내는 더 단순한 함수다. 이 둘은 거의 모든 지점에서 가중치를 살짝만 옮겨도 출력이 따라 움직여서, 그 방향과 크기를 계산해 낼 수 있다.
...
게이트의 진리표는 사실 입력 네 점을 0인 편과 1인 편, 두 편으로 가르는 문제다. 이 "점을 두 편으로 가른다"가 신경망이 푸는 문제의 기본형이다. 스팸 메일인지 아닌지, 사진에 찍힌 게 고양이인지 아닌지도 입력을 어떤 좌표 공간의 점 하나로 보고 그 점이 어느 편에 속하는지 가르는 문제로 바뀐다. AND, OR, NAND, XOR 네 게이트는 입력이 딱 네 점뿐인, 그 문제의 가장 작은 버전일 뿐이다.

가장 작은 버전 안에서도 갈린다. 넷 다 0과 1만 출력하는 건 같지만, 그중 셋(AND, OR, NAND)은 되고 XOR 하나만 안 된다. 차이는 **입력 네 점 중에서 1인 점과 0인 점을 직선 하나로 갈라놓을 수 있는가**에 있다.
...
가중합을 아무리 조절해도 직선 하나로 표현되는 경계밖에 못 그리기 때문에, 단일 퍼셉트론으로는 XOR에 대응하는 $w_1, w_2, \theta$가 존재하지 않는다. 실제 데이터에서 이런 배치가 드문 예외가 아니라는 게 문제다. 두 편이 서로 뒤섞여 마주 보는 모양은 얼마든지 나올 수 있고, XOR은 그중 가장 작고 명확한 사례일 뿐이다. 뉴런 하나가 가진 도구가 직선 하나뿐이라면, 이런 배치를 만나는 순간 그 뉴런은 통째로 못 쓰게 된다.
...
이걸 좌표 위에서 다시 보면 왜 되는지 더 분명해진다. NAND가 1을 내보내는 영역은 사각형에서 $(1,1)$ 구석의 삼각형 하나만 뺀 나머지 전체다. OR가 1을 내보내는 영역은 $(0,0)$ 구석의 삼각형만 뺀 나머지 전체다. 이 두 영역을 동시에 만족하는 부분, 즉 두 영역의 겹치는 자리를 구하면 두 구석 삼각형이 전부 빠지고 남는 자리는 $(0,1)$과 $(1,0)$ 두 점뿐이다. 이게 정확히 XOR이 1이어야 하는 자리다. 직선 하나로는 못 그리던 모양(마주 보는 대각선 두 점만 남기기)을, 직선 두 개가 각자 잘라낸 영역을 겹치는 것으로 만들어낸 것이다. 층을 하나 늘린다는 것은 산수로 보면 계산을 한 단계 더 거치는 것이지만, 도형으로 보면 직선을 하나 더 긋고 그 결과를 겹치는 것과 같다.
...
지금까지 만든 LLM은 여전히 12절에서 말한 다음 토큰 예측만 한다. "자율적으로 도구를 써서 응답한다"는 말은 신경망 안에 새 기능이 생겼다는 뜻이 아니라, **그 LLM 바깥에 도구를 실행하고 결과를 다시 넣어주는 반복문이 둘러싸였다**는 뜻이다. 이 반복문과 그걸 감싼 LLM을 합쳐서 에이전트라 부른다.

```mermaid
flowchart TB
    U["사용자 질문"] --> L["LLM<br/>지금까지 쌓인 텍스트로 다음 토큰 예측"]
    L -->|"출력이 도구 호출 형식"| T["도구 실행<br/>WebSearch / WebFetch / RAG"]
    T -->|"결과를 텍스트로 컨텍스트에 추가"| L
    L -->|"출력이 그냥 답변"| A["사용자에게 응답"]
```

LLM이 뱉는 출력은 여전히 텍스트 하나다. 다만 학습 데이터에 "지금은 직접 답하지 말고 이런 형식으로 도구를 요청해라"라는 패턴도 같이 가르쳐뒀기 때문에, 어떤 문맥에서는 이 출력이 `WebSearch(query: "쿠버네티스 최신 버전")` 같은 형식을 띤다. LLM 자신은 이 요청을 실행하지 못한다. LLM을 감싼 평범한 프로그램(에이전트 하니스)이 출력을 읽다가 도구 호출 형식을 감지하면, 그때 실제 검색 API를 부르거나 URL을 가져오거나 13절의 벡터 데이터베이스를 검색한다. 이 실행 부분은 신경망이 아니라 일반적인 함수 호출 코드다.

도구가 돌려준 결과는 다시 텍스트로 바뀌어 지금까지의 대화 뒤에 그대로 덧붙는다. 그리고 늘어난 텍스트 전체를 가지고 LLM을 처음부터 다시 호출한다. LLM 입장에서는 "질문 + 방금 도구가 찾아준 결과"가 전부 하나의 긴 입력일 뿐이고, 하는 일은 여전히 다음 토큰 예측이다. 이 과정을 LLM의 출력이 도구 호출을 요구하지 않을 때까지 반복하는 것이 에이전트 루프다. 몇 번 반복할지 미리 정해두지 않고, 매번 LLM의 출력을 보고 계속할지 멈출지를 그때그때 판단한다는 점이 "자율적"이라는 말의 실체다.

프롬프트 엔지니어링과 컨텍스트도 이 반복문에 무엇을 넣을지의 문제로 정리된다. 프롬프트 엔지니어링은 시스템 지시문을 어떻게 써서 LLM이 어떤 상황에 어떤 도구를 고르게 만드느냐의 문제이고, 이 지시문도 결국 7절의 토큰화와 임베딩을 거쳐 신경망에 들어가는 텍스트일 뿐 별도 채널이 아니다. 컨텍스트는 지금까지 오간 대화와 도구 결과가 쌓인 것이고, 8절에서 다룬 위치 임베딩 표의 최대 위치 수가 그대로 텍스트를 얼마나 쌓을 수 있는지의 한계(컨텍스트 윈도우)가 된다. RAG, WebFetch, WebSearch는 이 반복문 안에서 성격이 같은 도구다. 셋 다 LLM이 원래 모르거나 못 하는 정보를 외부에서 가져와 컨텍스트에 얹어주는 방식으로 보강한다. RAG는 13절의 벡터 데이터베이스에서, WebSearch는 검색엔진에서, WebFetch는 특정 URL에서 가져올 뿐, 텍스트로 바뀌어 컨텍스트에 얹히는 방식은 동일하다.
...
**프롬프트 엔지니어링, 컨텍스트, RAG, WebFetch, WebSearch가 맞물려서 자율적으로 응답하는 에이전트는 어떻게 동작하는가.** LLM 자체는 여전히 다음 토큰 예측만 한다. 그 출력이 도구 호출 형식이면 LLM 바깥의 평범한 코드가 실제 검색이나 조회를 실행하고, 결과를 텍스트로 바꿔 컨텍스트에 이어붙인 다음 LLM을 다시 호출한다. 이걸 도구 호출이 더 없을 때까지 반복하는 것이 에이전트 루프고, 프롬프트 엔지니어링은 이 반복문에 무엇을 먼저 넣을지, RAG/WebFetch/WebSearch는 이 반복문 중간에 무엇을 가져와 넣을지의 문제로 정리된다.

큰 그림으로 돌아가면, 가중치 하나에서 시작해 뉴런과 신경망으로, 토크나이저의 `encode()`가 텍스트를 토큰 ID로, 임베딩이 그 ID를 벡터로 바꾸고, Transformer의 인코더/디코더가 그 벡터를 처리해 BERT와 GPT로 갈리고, `decode()`가 다시 텍스트로 돌려주고, RAG가 그 앞단에서 참고 문서를 붙여주기까지 지도 위 칸이 전부 채워졌다. 그 위에 도구 호출 반복문을 씌운 것이 에이전트다. LangChain 컴포넌트는 아직 손으로 돌려본 게 아니라서 다음 실습 노트북에서 직접 확인하고 정리할 생각이다.
