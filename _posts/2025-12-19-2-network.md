...
**각 단계의 의미:**
1. **SYN**: 클라이언트가 ISN(Initial Sequence Number) 전송
2. **SYN-ACK**: 서버가 자신의 ISN 전송 + 클라이언트 ISN 확인
3. **ACK**: 클라이언트가 서버 ISN 확인

**꼬리 질문**: "4-way handshake는 언제 사용하나요?"
→ **연결 종료 시** 사용합니다. FIN → ACK → FIN → ACK

### Q2: NAT가 필요한 이유와 종류는?

**A:** IPv4 주소 부족 해결과 내부 네트워크 보호를 위해 사용합니다.

| NAT 유형 | 설명 | 사용 사례 |
|---------|------|----------|
| **SNAT** | Source NAT (출발지 주소 변환) | 내부 → 외부 통신 |
| **DNAT** | Destination NAT (목적지 주소 변환) | 포트 포워딩, 로드밸런싱 |
| **Masquerade** | 동적 IP용 SNAT | 홈 라우터, 동적 IP 환경 |

**Kubernetes에서의 NAT:**
- **NodePort**: DNAT로 노드 포트 → Pod 포트 변환
- **SNAT**: Pod → 외부 통신 시 노드 IP로 변환
...
### Q3: iptables의 체인과 테이블을 설명해주세요.

**A:**

**5개의 체인 (패킷 흐름 지점):**
```
PREROUTING → (라우팅 결정) → INPUT (로컬 프로세스로)
                          → FORWARD → POSTROUTING (다른 인터페이스로)
OUTPUT (로컬에서 생성) → POSTROUTING
```

**4개의 테이블 (기능별 규칙):**
| 테이블 | 목적 | 체인 |
|--------|------|------|
| **filter** | 패킷 필터링 (기본) | INPUT, FORWARD, OUTPUT |
| **nat** | 주소 변환 | PREROUTING, OUTPUT, POSTROUTING |
| **mangle** | 패킷 헤더 수정 | 모든 체인 |
| **raw** | 연결 추적 제외 | PREROUTING, OUTPUT |

**Kubernetes에서의 활용:**
- kube-proxy는 iptables 모드에서 Service → Pod 라우팅에 nat 테이블 사용
- NetworkPolicy는 filter 테이블로 트래픽 제어

### Q4: VXLAN이 필요한 이유는?

**A:** L2 네트워크를 L3 위에 오버레이하여 대규모 멀티테넌트 환경을 지원합니다.

**VLAN의 한계:**
- VLAN ID 12비트 → 최대 4,096개
- L2 도메인 내에서만 동작
...
**꼬리 질문**: "Pod에서 외부 도메인 조회 시 흐름은?"
1. Pod → CoreDNS (cluster.local 아님)
2. CoreDNS → 외부 DNS (forward 설정에 따라)
...
- [TCP/IP Illustrated](https://www.kohala.com/start/tcpipiv1.html)
...
> **원문 ([Kubernetes Documentation - Cluster Networking](https://kubernetes.io/docs/concepts/cluster-administration/networking/)):**
...
**번역:**
클러스터의 모든 Pod는 고유한 클러스터 전체 IP 주소를 받는다. 이는 Pod 간에 명시적으로 링크를 생성할 필요가 없으며 컨테이너 포트를 호스트 포트에 매핑하는 것을 거의 처리할 필요가 없음을 의미한다.

> **원문 ([RFC 793 - TCP](https://www.rfc-editor.org/rfc/rfc793)):**
> TCP provides a reliable, in-sequence delivery service for a stream of bytes between two endpoints. TCP segments are encapsulated in IP datagrams.

**번역:**
TCP는 두 엔드포인트 간의 바이트 스트림에 대해 신뢰할 수 있는 순차적 전달 서비스를 제공한다. TCP 세그먼트는 IP 데이터그램에 캡슐화된다.

> **원문 ([CNI Specification](https://github.com/containernetworking/cni/blob/main/SPEC.md)):**
> CNI (Container Network Interface), a Cloud Native Computing Foundation project, consists of a specification and libraries for writing plugins to configure network interfaces in Linux containers.

**번역:**
CNI(Container Network Interface)는 Cloud Native Computing Foundation 프로젝트로, Linux 컨테이너에서 네트워크 인터페이스를 구성하는 플러그인을 작성하기 위한 명세와 라이브러리로 구성된다.

## 다음 단계

- [컨테이너 기초](/container/container-basics) - 컨테이너 기술 심화
- [Kubernetes Service](/kubernetes/kubernetes-10-service) - Kubernetes 네트워킹
