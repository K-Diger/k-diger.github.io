...
지금 하는 일은 1인 데브옵스로 개발/스테이지/운영 세 환경, 70여 대 서버를 혼자 보는 것이다. 그중 일부(37대)를 Docker Compose에서 Kubernetes(22대)로 옮기는 작업을 진행 중이고, 아직 절반 이상은 Compose로 돈다. 이 글은 그 전환 과정에서 설계와 실제가 갈렸던 지점을 정리한 것이다.

먼저 정리하고 갈 오해가 두 가지 있다.

**"오토스케일링 때문에 옮겼다"는 아니다.** 지금 클러스터에 HPA는 켜 두었지만 최소와 최대 replica가 같아서 실제로는 안 늘어난다. 켜 둔 이유는 트래픽에 따라 얼마나 늘려야 하는지 판단할 지표를 쌓기 위해서지, 지금 당장 자동으로 늘리려는 게 아니다.
...
실제로 옮긴 이유는 다른 데 있었다. 서버에 들어가서 직접 배포 스크립트를 실행하고, 라우팅 규칙은 게이트웨이 관리 화면에만 있고, 방화벽에 뭘 열었는지는 결재 문서에만 있고, 어느 앱이 자원을 얼마나 쓰는지는 어디에도 없었다. 이렇게 흩어진 규칙을 한곳에 모으고 사람이 아니라 도구가 지키게 만드는 게 목적이었다.

전환을 진행하면서 실측한 수치는 이렇다. 사람이 반복적으로 손대던 작업 시간이 항목별로 75~100% 줄었고(배포 자동화로 월 27시간 상당 절감, 설정 반영 소요 75~90% 감소), 장애 감지부터 복구까지 걸리던 시간이 30분대에서 수십 초로 줄었다. 다만 이 숫자는 최소~최대 구간이지 평균이 아니다. 시스템마다 편차가 크다는 뜻이고, 그 편차 자체가 사람이 기억해서 처리하던 방식의 한계를 보여준다.

정리하면서 확인하고 싶었던 것들이다.

- 기존 API 게이트웨이를 걷어낼지 유지할지 무엇을 기준으로 판단했는가?
- 로드밸런서부터 파드까지 트래픽이 실제로 어떻게 도달하는가?
- 인증서 갱신과 배포 롤백 같은 반복 작업은 어떻게 자동화했는가?
- 장애 1차 대응까지 자동화할 만한 근거가 있었는가?
...
배포 파이프라인은 SSH로 접속해 컨테이너를 순차적으로 내리고 올리는 롤링 방식이었다. 이걸 두고 "무중단 배포"라고 불렀는데, 이 표현을 그대로 쓰다가 스스로 정정한 적이 있다.
...
1. **배포 전략**(카나리, 블루/그린). 새 버전과 구 버전을 동시에 띄우고 트래픽을 서서히 옮기는 문제다.
2. **롤링 업데이트 자체의 중단 가능성.** 컨테이너를 내리기 *전에* 게이트웨이 라우팅에서 먼저 빼는가의 문제다.
...
설계 문서를 실제 구성에 맞춰 고치면서 지금까지 이해해온 트래픽 경로가 틀렸다는 걸 인정해야 했다. MetalLB 기준으로 트래픽 흐름을 설명하려다 막힌 적이 있는데, 막힌 이유는 MetalLB를 안 써서가 아니라 **각 계층이 정확히 무슨 책임을 지는지 구분하지 못했기 때문**이었다. 그 계층을 정리하면 이렇다.
...
### 표준 NetworkPolicy 대신 Cilium 전용 리소스를 쓴 이유

네트워크 정책은 기본을 전부 차단하고 필요한 경로만 여는 방식(default-deny)으로 설계했다. 그런데 이걸 표준 Kubernetes `NetworkPolicy`가 아니라 Cilium 전용 리소스(`CiliumNetworkPolicy`)로 작성해야 했던 이유가 있었다.

표준 `NetworkPolicy`에서 `namespaceSelector`와 `podSelector`를 AND 조건으로 조합하면(특정 네임스페이스의 특정 라벨을 가진 Pod에서만 오는 트래픽을 허용), 그 조합이 커널의 eBPF 정책 맵에 반영되는 과정에서 조용히 누락되는 문제를 겪었다. 에러가 나지 않고 트래픽만 막히기 때문에 원인을 좁히는 데 시간이 걸렸다. 회피 방법은 `fromEndpoints`/`toEndpoints` 하나의 selector 안에 네임스페이스 라벨과 Pod 라벨을 동시에 명시하는 것이었다. 두 selector를 나눠 쓰지 않고 하나로 합치면 이 경로를 타지 않는다.

이 경험에서 배운 건, CNI를 바꾸는 결정은 "기능이 더 많다"만으로 끝나지 않는다는 것이다. 표준 리소스가 그 구현체에서 알려진 결함을 갖고 있을 수 있고, 그 결함을 알아채려면 정책이 "설정한 대로 동작하는지"를 실제로 검증하는 절차가 있어야 한다. [Hubble](https://docs.cilium.io/en/stable/observability/hubble/) 같은 관측 도구로 "의도한 트래픽이 실제로 통과하는지, 막힌 트래픽이 의도한 대로 막혔는지"를 눈으로 확인하는 습관이 여기서 생겼다.

한때 "kube-proxy가 일종의 CNI 역할을 한다"고 잘못 이해하고 있었는데 이게 정확히 틀린 지점이다. kube-proxy와 CNI는 둘 다 네트워킹을 다루지만 책임이 다르다. 이 세 계층을 분리해서 그릴 수 있으면, "L4가 Pod IP를 아는가 모르는가"라는 질문 자체가 성립하지 않는다는 걸 알 수 있다. L4는 애초에 그걸 알 필요가 없는 계층이기 때문이다.

## Ansible 70여 대: 인증서와 롤백

Kubernetes로 옮기지 않은 나머지 서버는 여전히 [Ansible](https://docs.ansible.com/)로 관리한다. 여기서 반복적으로 손이 간 두 가지가 인증서 갱신과 배포 롤백인데, 자동화하게 된 계기는 실제로 겪은 장애였다.

### 인증서를 손으로 복사하다 게이트웨이가 전면 다운된 사고

인증서를 자동화하기 전에는 서버 두 대에 사람이 직접 새 인증서 파일을 복사해 넣는 방식이었다. 그런데 한 대는 파일 권한이 잘못 설정됐고, 다른 한 대는 인증서와 개인키 짝이 맞지 않는 상태로 복사됐다. 두 대가 동시에 문제를 일으키면서 그 뒤에 있던 모든 서비스로의 진입 경로가 막혔다.

복구가 오래 걸린 이유는 원인이 한 겹이 아니었기 때문이다. 권한 오류와 짝이 안 맞는 인증서라는 서로 다른 두 문제가 겹쳐 있었고, 사람이 한 서버씩 들어가서 한 겹씩 벗겨내야 했다. 이 사고를 계기로 세 가지를 바꿨다.

1. **인증서 배포를 코드로 관리한다.** 저장소 한 곳에 인증서 파일을 등록해두고, 배포는 Ansible 플레이북이 전 서버에 동일하게 적용한다. 사람이 파일을 복사하는 단계 자체를 없앴다.
2. **무엇을 언제 왜 바꿨는지 이력이 남는다.** 변경이 커밋으로 남으니 되돌릴 때 무엇을 되돌릴지부터 찾는 시간이 사라진다.
3. **잘못되면 직전 상태로 즉시 복귀할 수 있어야 한다.** 배포 자체를 되돌릴 수 있는 구조로 만들어서, 잘못된 인증서가 배포됐다는 걸 알아챈 순간 바로 이전 상태로 되돌릴 수 있게 했다.

이 사고가 자동화를 도입한 계기이자, 자동화의 성공 기준이 됐다. "사람이 두 번 다시 인증서 파일을 직접 서버에 복사하지 않는다"가 그 기준이다.
...
**게이트웨이를 걷어낼지 유지할지의 기준.** 개발자들이 이미 만들어둔 커스텀 플러그인을 다른 게이트웨이로 옮기는 공수가 기준이었다. 공수가 큰 부분(인증, 감사 로그)은 유지하고, 정리가 덜 된 나머지 경로만 Envoy Gateway로 옮겼다.

**트래픽이 실제로 도달하는 경로.** L4는 어떤 노드가 VIP를 받을지만 알고, kube-proxy(또는 Cilium)가 Service와 Endpoints를 watch하며 실제 Pod로 DNAT하는 규칙을 관리하고, CNI는 그 아래 Pod 네트워크 자체를 담당한다. 이 세 계층의 책임을 분리해서 그릴 수 있어야 L4가 Pod IP를 아는지 모르는지 같은 질문 자체가 성립하지 않는다는 걸 알 수 있다.

**반복 작업을 자동화한 방법.** 인증서는 저장소 한 곳에 등록해두고 Ansible 플레이북이 전 서버에 동일하게 적용하며, 잘못되면 직전 상태로 즉시 되돌릴 수 있게 했다. TLS 만료는 서버 안 스크립트 대신 blackbox_exporter로 밖에서 확인해서 "체크가 안 됐다"는 상태 자체가 드러나게 만들었다.

**장애 1차 대응을 자동화한 근거.** 비용이 예상보다 훨씬 작았다는 점이다. 저렴한 모델 기준으로 진단 1회에 원 단위 비용이 들고, 하루 알림 100건이어도 하루 비용이 커피 한 잔에도 못 미친다. 다만 이 자동화가 기존 보안 절차를 우회하는 경로가 됐다는 걸 뒤늦게 알아챘다는 흠은 남는다.

이 전환에서 계속 반복된 건 설계와 실제가 갈리는 지점을 인정하는 일이었다. MetalLB로 설계했지만 실제로는 F5와 NodePort가 자리 잡았고, 무중단이라고 부르던 배포가 사실은 짧은 실패 구간이 있는 롤링 배포였다. 그때마다 문서를 실제 구성에 맞춰 다시 쓰는 게 이 작업의 절반이었다.
