...
전환을 진행하면서 실측한 수치는 이렇다. 사람이 반복적으로 손대던 작업 시간이 항목별로 75~100% 줄었고(배포 자동화로 월 27시간 상당 절감, 설정 반영 소요 75~90% 감소), 장애 감지부터 복구까지 걸리던 시간이 30분대에서 수십 초로 줄었다. 다만 이 숫자는 최소~최대 구간이지 평균이 아니다 — 시스템마다 편차가 크다는 뜻이고, 그 편차 자체가 "사람이 기억해서 처리하던 방식"의 한계를 보여준다.
...
이 결정 자체보다 중요한 건, 설계 문서를 실제 구성에 맞춰 정정하면서 "지금까지 내가 이해해온 트래픽 경로가 틀렸다"는 걸 인정해야 했던 순간이다. MetalLB 기준으로 트래픽 흐름을 설명하려다 막힌 적이 있는데, 돌이켜보면 막힌 이유는 MetalLB를 안 써서가 아니라 **각 계층이 정확히 무슨 책임을 지는지 구분하지 못했기 때문**이었다. 그 계층을 정리하면 이렇다.
