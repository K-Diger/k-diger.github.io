...
쿠버네티스 코드를 읽다가 DRA 플러그인의 gRPC 연결을 다루는 부분에서 어색한 곳을 발견했다. **같은 파일 안에서 연결을 만드는 방식이 두 가지로 갈려 있었다.**

한쪽은 플러그인을 등록할 때 미리 연결을 만들어두고, 다른 쪽은 처음 호출될 때 만들어 두는 방식이었다. 왜 다른지 이유를 못 찾았고, 뒤쪽 방식에는 여러 고루틴이 동시에 들어올 때 문제가 생길 여지가 있어 보였다.

정리해서 이슈를 올리고 패치를 보냈고, [PR #133964](https://github.com/kubernetes/kubernetes/pull/133964)가 v1.36 마일스톤에 병합됐다.

여기서는 그 과정과 배경을 정리한다.

정리하면서 확인하고 싶었던 것들이다.

- DRA는 기존 Device Plugin으로 안 되는 무엇을 하려고 나왔는가?
- 한 컴포넌트 안에서 연결 관리 방식이 갈리면 실제로 무엇이 문제인가?
- 지연 초기화는 흔한 패턴인데 왜 여기서는 맞지 않았는가?
- 큰 프로젝트에 패치를 보내는 과정에서 무엇을 확인해야 했는가?
...
DRA API는 매번 `getOrCreateGRPCConn()`을 호출한 뒤 새 클라이언트를 만들고, Health API는 별도의 지연 초기화로 `healthClient`를 관리한다. **두 경로가 연결 관리 방식을 공유하지 않는다는 점**이 문제의 출발점이었다.
...
**DRA가 무엇을 하려고 나왔는가.** 기존 Device Plugin은 "이 노드에 GPU가 4개 있다"까지만 말할 수 있었다. 어떤 모델인지, 메모리가 얼마인지, 여러 파드가 나눠 쓸 수 있는지 같은 것을 표현할 방법이 없었다. DRA는 **자원을 개수가 아니라 속성을 가진 객체로 다루게** 해서 그 표현력을 확보한다.

**연결 관리 방식이 갈리면 무엇이 문제인가.** 두 가지다. 읽는 사람이 **어느 쪽이 의도된 것인지 판단할 수 없다.** 그리고 각각이 다른 시점에 연결을 만들기 때문에, **한쪽에서 고친 문제가 다른 쪽에는 그대로 남는다.** 실제로 재연결 처리와 종료 처리가 두 경로에서 다르게 되어 있었다.
...
`grpc.Dial`에서 `grpc.NewClient`로 옮긴 것도 함께 다뤘는데, **이건 별개의 변경이므로 왜 같은 PR에 넣었는지도 설명이 필요했다.** 연결 생성 지점을 하나로 모으는 작업이라 같은 자리를 건드리기 때문이었다.

돌아보면 남는 것은 **큰 프로젝트일수록 사소해 보이는 코드가 넓게 퍼진다는 것**이었다. kubelet은 모든 노드에서 도는 컴포넌트다. 여기의 연결 하나가 잘못 남으면 노드 수만큼 곱해진다. 그래서 "이 정도는 괜찮다"의 기준이 평소보다 훨씬 엄격해야 했다.
