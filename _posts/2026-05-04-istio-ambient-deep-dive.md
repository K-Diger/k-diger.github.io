...
- [ ] [L7 Authorization Policy](https://istio.io/latest/docs/ambient/usage/l7-features/)
...
이 계층적 접근 방식을 통해 Istio를 더 점진적으로 도입할 수 있으며, 메시 미참여 상태에서 보안 L4 오버레이, 그리고 전체 L7 처리로 원활하게 전환할 수 있다.
...
ztunnel(Zero Trust Tunnel) 컴포넌트는 특별히 구축된 노드별 프록시다.

ztunnel은 워크로드를 안전하게 연결하고 인증하는 역할을 담당한다.
...
워크로드의 HTTP 트래픽을 종료하거나 워크로드의 HTTP 헤더를 파싱하지 않는다.
...
ztunnel은 HBONE(HTTP CONNECT 기반 터널링 프로토콜)을 통해 트래픽을 라우팅한다.

Ambient 메시 내 워크로드 간의 모든 인바운드 및 아웃바운드 L4 TCP 트래픽은 HBONE, ztunnel, x509 인증서를 통한 mTLS를 사용하여 데이터 플레인에 의해 보호된다.
...
Waypoint 프록시는 Envoy 프록시의 배포이며, Istio가 사이드카 모드에서 사용하는 것과 동일한 엔진이다.

Waypoint 프록시는 정의된 워크로드 집합에 L7 처리를 추가하기 위한 Envoy 기반 프록시의 선택적 배포다.
...
Ambient 메시 내의 Pod이 아웃바운드 요청을 보내면, 해당 요청은 투명하게 노드 로컬 ztunnel로 리다이렉트된다.

Ambient 메시 내의 Pod이 인바운드 요청을 수신하면, 해당 요청은 투명하게 노드 로컬 ztunnel로 리다이렉트된다.
...
ztunnel은 인가 정책을 적용한 후 트래픽을 전달한다.
...
Waypoint 프록시는 HBONE 트래픽만 수신하며, L7 로드밸런싱과 엔드포인트 간 라우팅을 수행한다.
...
워크로드는 메시 외부, 메시 내부(L4), Waypoint 포함 메시 내부(L7)의 세 가지 카테고리로 분류된다.
...
ztunnel은 자신이 위치한 특정 노드에서 실행 중인 ID에 대한 인증서만 요청할 수 있다.
...
ztunnel은 Istio 표준 TCP 메트릭의 전체 집합을 방출한다.
...
Ambient 모드의 대부분의 기능은 ztunnel 노드 프록시가 제공한다(L4만 해당).
...
`istio.io/waypoint-for` 라벨의 트래픽 유형은 service(기본값), workload, all, none이다.
...
`istio.io/use-waypoint` 라벨로 등록을 설정하며, 네임스페이스, 서비스, 또는 개별 Pod에 적용할 수 있다.
...
우선순위는 Pod 라벨 > Service 라벨 > Namespace 라벨 순서다.
...
크로스 네임스페이스 Waypoint 사용 시 `istio.io/use-waypoint`와 `istio.io/use-waypoint-namespace` 라벨을 함께 사용한다.

다른 네임스페이스에 배포된 Waypoint를 사용해야 할 때, 두 라벨을 조합하여 대상 Waypoint의 이름과 네임스페이스를 모두 지정한다.

### 7.6 Waypoint가 필요한 기능 목록

Waypoint가 필요한 기능: HTTP 라우팅 및 로드밸런싱, 서킷 브레이킹, 속도 제한, 장애 주입, 재시도, 타임아웃, L7 프리미티브 기반의 고급 인가 정책.
...
ztunnel은 트래픽 전달 전에 인가 정책을 적용한다.
...
Waypoint는 AuthorizationPolicy, RequestAuthentication, WasmPlugin, Telemetry 등의 L7 정책을 적용한다.
...
Waypoint는 엔드포인트 간 L7 로드밸런싱과 라우팅을 수행한다.
...
공식문서는 ztunnel을 노드마다 하나씩 두는 전용 프록시로, 워크로드를 안전하게 연결하고 인증하는 역할이라고 설명한다. Waypoint는 Envoy를 배포한 것이고, 사이드카 모드에서 쓰던 것과 같은 엔진이다. ([Ambient Overview](https://istio.io/latest/docs/ambient/overview/))

답변 방향이다. ztunnel은 DaemonSet으로 각 노드에 배포되는 L4 프록시로, mTLS 수립/종료, L4 AuthorizationPolicy 평가, TCP 텔레메트리 수집을 담당한다. Rust로 구현되어 경량이며, 의도적으로 L7 기능을 배제하여 보안 경계를 최소화했다. Waypoint Proxy는 Envoy 기반 L7 프록시로, HTTP 라우팅, 재시도, L7 인가 등 고급 기능이 필요한 경우에만 선택적으로 배포한다. 이 분리 구조로 대부분의 워크로드는 ztunnel의 L4 보호만으로 운영하고, L7이 필요한 소수의 워크로드에만 Waypoint를 추가하여 리소스 효율을 극대화한다.

**꼬리질문 1:** ztunnel이 L7을 처리하지 않는 보안적 이유는?

ztunnel은 노드 레벨에서 실행되므로 해당 노드의 모든 Pod 트래픽을 처리한다. 공식문서가 명시하듯 워크로드의 HTTP 트래픽을 종료하지도, HTTP 헤더를 파싱하지도 않도록 의도적으로 설계되었다. 이 프록시가 L7 데이터(HTTP 본문, 헤더)를 해석할 수 있다면, 노드 침해 시 공격자가 모든 Pod의 HTTP 레벨 데이터에 접근 가능해진다. L4만 처리하도록 제한함으로써 blast radius를 최소화한 것이다.
...
**사이드카를 없애면 누가 가로채는가.** 노드마다 하나씩 뜨는 ztunnel이 맡는다. 파드 안이 아니라 노드에 있으므로 파드 수와 무관하게 노드 수만큼만 뜬다. 트래픽 방향 전환은 커널 수준에서 일어나므로 애플리케이션은 여전히 아무것도 모른다.

**L4와 L7을 왜 나누는가.** 필요한 만큼만 쓰기 위해서다. 대부분의 워크로드에 필요한 것은 암호화와 신원 확인, 즉 L4 수준이다. HTTP 라우팅이나 재시도 같은 L7 기능이 필요한 워크로드는 소수다. **모두에게 L7 프록시를 붙이는 대신, L4는 노드 단위로 처리하고 L7이 필요한 곳에만 Waypoint를 추가한다.**

보안적인 이유도 있다. ztunnel은 노드의 모든 파드 트래픽을 다룬다. **여기서 HTTP 본문까지 해석할 수 있으면 노드 하나가 뚫렸을 때 그 노드의 모든 애플리케이션 데이터가 넘어간다.** L4만 처리하도록 제한해서 그 범위를 줄인 것이다.

**HBONE이 왜 필요한가.** 순수한 TCP 위에서 mTLS만 하면 "이 연결이 원래 어디로 가려던 것인지"를 담을 자리가 없다. ztunnel은 여러 파드의 트래픽을 대신 나르므로 그 정보가 필요하다. HTTP/2의 CONNECT를 쓰면 **터널 안에 목적지와 신원 같은 메타데이터를 실을 수 있고**, 하나의 TCP 연결에 여러 논리적 연결을 겹쳐 실을 수도 있다.
