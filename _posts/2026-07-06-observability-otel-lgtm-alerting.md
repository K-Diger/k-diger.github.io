...
상용 APM을 쓰다가 관측성 스택을 직접 세우게 됐다. 계기가 된 사고가 있었다. 업무 시스템 하나에서 메모리 사용량이 서서히 올라가다가 응답이 멈추고 결국 504 에러로 이어지는 연쇄 장애가 났는데, 지표는 상용 APM에, 로그는 별도의 로그 시스템에 나뉘어 있어서 "메모리가 왜 올라갔는지"와 "왜 멈췄는지"와 "왜 504가 났는지"를 하나의 흐름으로 이어 볼 방법이 없었다. 사후 분석 자체가 불가능했다. 이 경험이 지표·로그·트레이스를 한 곳에서 클릭 한 번으로 오갈 수 있어야 한다는 요구로 이어졌다.

도구를 켜는 것 자체는 문서대로 하면 되는데, 그 뒤에 답이 안 나오는 질문들이 남았다.
...
카디널리티 개념 자체는 위와 같지만, 정작 이걸 다른 사람에게 설명하면서 잘못된 예시를 든 적이 있다. "타임스탬프가 라벨로 들어오면 카디널리티가 높아진다"고 말했는데, 이건 틀린 예시다. 타임스탬프는 시계열의 값(포인트가 찍히는 x축)이지 라벨이 아니다. 카디널리티를 실제로 폭증시키는 건 위에서 든 `trace_id`, `user_id`, `request_id`처럼 **값이 무한히 늘어나는 필드를 라벨(차원)로 붙이는 것**이다. 개념(라벨 조합이 늘수록 시계열 수가 곱으로 늘어나 조회 성능이 떨어진다)은 맞게 짚었지만 예시가 틀리면서 이해도가 얕아 보였던 경험인데, 이후 Loki/Mimir 문서를 다시 읽으면서 "라벨=차원, 값이 늘수록 위험"이라는 원칙으로 재정리했다.
...
트레이스는 요청마다 생기기 때문에 전량 저장하면 비용이 감당이 안 된다. 그래서 Collector 단계에서 tail sampling(요청이 끝난 뒤, 결과를 보고 저장 여부를 정하는 샘플링)을 건다. 실무에서 정한 비율은 이렇다 — 에러로 끝난 요청은 100% 보존, 응답 시간이 임계치(예: 2초)를 넘긴 요청도 100% 보존, 나머지 정상 요청은 10%만 보존한다. 이렇게 하면 "장애 분석에 필요한 트레이스는 반드시 남아 있다"가 보장되면서 저장 비용은 크게 줄어든다. 다만 이 구조를 모르고 정상 요청의 `trace_id`로 조회하면 90% 확률로 "Not Found"가 뜨는데, 이건 이상이 아니라 샘플링에서 빠졌을 뿐이다 — 실제로 처음 이 구조를 도입했을 때 이걸 장애로 착각한 적이 있다.
