...
카디널리티 개념 자체는 위와 같지만, 정작 면접에서 이걸 설명하다가 잘못된 예시를 든 적이 있다. "타임스탬프가 라벨로 들어오면 카디널리티가 높아진다"고 말했는데, 이건 틀린 예시다. 타임스탬프는 시계열의 값(포인트가 찍히는 x축)이지 라벨이 아니다. 카디널리티를 실제로 폭증시키는 건 위에서 든 `trace_id`, `user_id`, `request_id`처럼 **값이 무한히 늘어나는 필드를 라벨(차원)로 붙이는 것**이다. 개념(라벨 조합이 늘수록 시계열 수가 곱으로 늘어나 조회 성능이 떨어진다)은 맞게 짚었지만 예시가 틀리면서 이해도가 얕아 보였던 경험인데, 이후 Loki/Mimir 문서를 다시 읽으면서 "라벨=차원, 값이 늘수록 위험"이라는 원칙으로 재정리했다.
