...
트레이스는 요청마다 생기기 때문에 전량 저장하면 비용이 감당이 안 된다. 그래서 Collector 단계에서 tail sampling(요청이 끝난 뒤, 결과를 보고 저장 여부를 정하는 샘플링)을 건다. 실무에서 정한 비율은 이렇다. 에러로 끝난 요청은 100% 보존, 응답 시간이 임계치(예: 2초)를 넘긴 요청도 100% 보존, 나머지 정상 요청은 10%만 보존한다. 이렇게 하면 장애 분석에 필요한 트레이스는 반드시 남아 있다는 게 보장되면서 저장 비용은 크게 줄어든다. 다만 이 구조를 모르고 정상 요청의 `trace_id`로 조회하면 90% 확률로 "Not Found"가 뜨는데, 이건 이상이 아니라 샘플링에서 빠졌을 뿐이다. 처음 이 구조를 도입했을 때는 이걸 장애로 착각한 적도 있다.
